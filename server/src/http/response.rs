...
use crate::http::common::{HTTPDate, HTTPVersion, Header, StatusCode};
    ...
    let user_request = request_headers
        ...
        .and_then(|value| {
            ...
            for value in parse.iter() {
                ...
                if let Ok(te) = <TransferEncoding as FromStr>::from_str(value.0) {
                    return Some(te);
                }
            }
    ...
    R: Read,
{
        ...
        H: Into<Header>,
    {
        ...
        if header.field.equiv(&"Content-Length") {
            if let Ok(val) = <usize as FromStr>::from_str(header.value.as_str()) {
