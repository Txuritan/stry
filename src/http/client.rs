...
use crate::http::common::{HTTPVersion, Method};
use crate::http::util::RefinedTcpStream;
use crate::http::util::{SequentialReader, SequentialReaderBuilder, SequentialWriterBuilder};
...
impl ClientConnection {
    ...
    fn read(&mut self) -> Result<Request, ReadError> {
        ...
        let request = crate::http::request::new_request(
            self.secure,
            method,
            path,
            version.clone(),
...
mod test {
    ...
    fn test_parse_request_line() {
        ...
        assert!(method == crate::http::Method::Get);
        assert!(path == "/hello");
        assert!(ver == crate::http::common::HTTPVersion(1, 1));
